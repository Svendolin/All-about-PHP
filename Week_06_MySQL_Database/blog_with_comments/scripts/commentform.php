<?php
/* KOMMENTARFORMULAR (Ausgelagert im Script) - Commentform basierend auf PHP
...
if( isset($_POST['author_name']) && isset($_POST['author_email']) && isset($_POST['comment_text']) ){
	...
	if($hasError == false){
        ...
        `comment_status`)
        VALUES
        ('{$articleID}' , '{$name}',  '{$email}', '{$text}', '0')"; // Status 
		// die ID wird automatisch erstellt in der Datenbank, wenn auto increment aktiviert ist. 
		// echo $query;

		$commentSaved = mysqli_query($connection, $query);
	
		// prüfen, ob der Kommentar ggespeichert wurde, dann alle weiteren Aktionen ausführen:
		if($commentSaved != false){
